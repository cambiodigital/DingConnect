...
75. Corrección visual en frontend público (`Paquetes disponibles`): se eliminó el recorte lateral del desplegable nativo del select de paquetes ajustando `overflow` en contenedores padre (`.dc-card` y `.dc-wizard`) para permitir render completo de opciones largas, y en móvil (`max-width: 640px`) se redujo tipografía/padding del control para mejorar legibilidad de labels extensos.
