...
- Precheck obligatorio antes de checkout (v2.8.39): el frontend llama `POST /wp-json/dingconnect/v1/precheck` al pulsar **Continuar**. Backend valida `GetAccountLookup`, disponibilidad del SKU con `GetProducts`, compatibilidad operador/país/región, monto contra bundle y producto live, settings/factura, balance disponible y `SendTransfer` con `ValidateOnly=true`. Solo si la validación es concluyente se emite un `precheck_token` temporal de 5 minutos.
- Ajuste por cobertura real de lookup/catálogo (v2.8.43): `GetAccountLookup` se usa como señal auxiliar para detectar país/proveedor cuando Ding lo devuelve, pero un lookup vacío, temporalmente no disponible o con país/proveedor distinto ya no bloquea por sí solo. Si `GetProducts` falla o no devuelve el SKU y existe bundle guardado, el backend usa ese bundle como respaldo de reglas locales y deja la decisión final en `SendTransfer ValidateOnly=true`.
- Respuesta UX de `/precheck` (v2.8.43): los rechazos funcionales esperados (número no recargable, país no coincidente, importe no permitido, producto no disponible) se devuelven como `ok:false` con HTTP 200 para que el frontend muestre feedback limpio al cliente; errores temporales reales (`RateLimited`, timeouts, servicio no disponible) conservan HTTP 429/5xx y `retryable:true`.
- Validación de número por regex (v2.8.44): antes de emitir `precheck_token`, el backend valida `AccountNumber` contra `ValidationRegex` enriquecido desde proveedor/producto/bundle. Si Ding no aporta regex pero el producto es móvil Colombia, se aplica fallback conservador `^(57)?3[0-9]{9}$` para evitar el falso positivo que en producción terminaba como `AccountNumberInvalid / AccountNumberFailedRegex` después del pago.
- Guardia anti-bypass en carrito: `POST /wp-json/dingconnect/v1/add-to-cart` exige `precheck_token` vigente y con fingerprint coincidente (`account_number`, país, SKU, bundle, monto, moneda, settings y `bill_ref`). Si el token falta, expiró o los datos cambiaron, no se añade al carrito ni se cobra.
- Alineación oficial de respuestas (v2.8.40): para DingConnect, `ResultCode=1` significa éxito y `ResultCode=2` éxito con advertencia. El precheck acepta solo validación concluyente sin `ErrorCodes`, y el frontend ya no trata `ResultCode=1` como error en `EstimatePrices`/`LookupBills`.
- Trazabilidad API: las llamadas salientes incluyen `X-Correlation-Id`, usando `DistributorRef` cuando existe para facilitar diagnóstico cruzado con DingConnect.
- Comprobante cliente (v2.8.41): modal thank-you, PDF y resultado directo muestran `TransferRef` como `ID de transacción`. Si la recarga queda pendiente y aún no existe `TransferRef`, el cliente ve `Pendiente de confirmación`; `DistributorRef` se muestra aparte como `Referencia interna` solo para soporte.
...
- Control de etapa por pasarela: en `Credenciales` existe selector de hook de despacho (`payment_complete`, `processing`, `completed`) con valor por defecto y override por gateway para adaptar pasarelas manuales vs automáticas.
- Guardia anti-falso-éxito por modo de pruebas: si el modo efectivo queda en `ValidateOnly`, WooCommerce no ejecuta despacho real y marca el ítem como `validate_only` con nota explícita, evitando que una validación técnica se interprete como recarga exitosa.
- Confirmación estricta de referencia Ding: un estado `Complete` sin referencia confirmada (`TransferRef` vacío o `0`) no se trata como éxito terminal; pasa a `pending_confirmation` y obliga conciliación vía `ListTransferRecords`.
...
- Política de estado de pedido post-pago: una vez pagada la orden, el estado WooCommerce se sincroniza por resultado agregado de recargas DingConnect: `completed` cuando todos los ítems quedan en éxito confirmado (estado exitoso + `TransferRef` confirmado), `processing` cuando hay ítems pendientes (`submitted/pending/processing/pending_retry/pending_confirmation`) y `on-hold` cuando existe al menos un ítem con error definitivo o escalado de soporte. La transición se ejecuta con `WC_Order->update_status()` para disparar hooks estándar de WooCommerce.
...
- Persistencia de orden hardening: en alta/edición de landing el submit genera `bundle_order` explícito por `bundle_id` en función del orden real de filas, para evitar pérdida de reordenamiento cuando el navegador reordena controles del formulario.
- Operación compacta en modal de shortcodes creados: el modal de edición usa una única tabla operativa para gestionar bundles de la landing sin salir de la vista, con botón por fila `Añadir`/`Quitar`, filtros por país/tipo/buscador y filtro de estado (`Todos`, `Solo en landing`, `Disponibles para añadir`), conservando reordenamiento drag and drop y destacado opcional.
- Edición directa por producto desde `Shortcodes dinámicos`: dentro del modal `Editar shortcode dinámico`, al hacer click en la celda `Producto` de un bundle se abre su modal de `Editar producto` para modificar ese registro de catálogo de forma inmediata.
- Persistencia de contexto tras guardado: al enviar `Guardar cambios del shortcode` desde el modal de edición, el backend redirige explícitamente a `dc_tab=tab_landings` y `dc_landings_subtab=shortcodes`, manteniendo al operador en la lista de shortcodes dinámicos sin reabrir el modal.
...
- Resiliencia ante caché de página en landings: el shortcode expone `landing_key` en el DOM y el frontend consulta configuración runtime por REST (`GET /wp-json/dingconnect/v1/landing-config?landing_key=...`) antes de buscar paquetes para usar el set actualizado de `bundle_ids` y `featured_bundle_id`.
- Flujo público de la landing: el shortcode `dingconnect_recargas` ya no expone un índice numerado visible; resuelve el recorrido en dos pasos operativos. Paso 1: número + selector de paquetes disponibles para la landing, con ficha visual de Beneficios recibidos, Operador, Monto y País ISO. Paso 2: confirmación con nombre del paquete, beneficios y precio antes de derivar a WooCommerce o a la recarga directa (sin mostrar operador, número ni país).
- Regla UX de `Paquetes disponibles`: cuando existen múltiples paquetes, la ficha de detalle permanece oculta hasta selección explícita del usuario; solo si hay un único paquete se auto-selecciona y se muestra automáticamente.
...
- La personalización visual por shortcode (estilos/vista previa en admin y CSS inline por instancia) fue retirada del plugin para mantener un único diseño operativo mientras se define una nueva versión de esa capacidad.
...
- Regla de monto manual por producto de rango: además del control global `manual_amount_mode` en `Config`, cada bundle de rango define `allow_manual_amount`; si está desactivado, frontend y backend fuerzan monto fijo (`send_value`) aunque el SKU admita rango en DingConnect.
- Hardening de render admin para edición de bundle: al calcular estados de checkboxes en formularios, nunca usar `array_key_exists` sobre variables potencialmente nulas (`$editing_bundle`); validar primero `is_array(...)` para evitar `TypeError` fatal en PHP 8+ y caída total de `admin.php?page=dc-recargas`.
...
- Regla de prioridad de precio al carrito: en `POST /add-to-cart` el backend debe resolver `Precio al público` desde el bundle guardado (preferentemente por `bundle_id`) y solo usar `Coste Ding` si el precio público resultante es 0.
- Regla de validación de monto fijo en carrito: en `POST /add-to-cart`, la validación de `send_value` debe priorizar el bundle exacto (`bundle_id`) cuando venga en el payload; usar solo `sku_code + country_iso` puede cruzar contra otro bundle del mismo SKU y bloquear checkout con un importe fijo incorrecto.
- Compatibilidad de bundles antiguos: cuando un producto guardado no tenga `description`, el flujo debe usar fallback `receive` (campo legacy) para poblar `Description` y `bundle_benefit` en checkout, evitando perder el texto de `Beneficios recibidos` tras migraciones históricas.
- Regla de rehidratación en sesión para checkout: si un item DingConnect vuelve desde sesión sin `bundle_benefit`, `bundle_label`, `provider_name` o precio comercial, WooCommerce debe reinyectar esos datos desde el bundle guardado usando `bundle_id` o fallback `sku_code + country_iso`, para que checkout/cart mantengan el texto operativo correcto y no dependan solo del payload original de `add-to-cart`.
- Fuente canónica para rehidratación de bundles guardados: checkout/cart deben leer los bundles desde `get_option('dc_recargas_bundles')`; usar `dc_recargas_options` para esa búsqueda rompe la recuperación de `Beneficios recibidos` y otros metadatos del producto en carritos restaurados.
- Convención de copy en `Tu pedido`: el `account_number` de la recarga debe mostrarse con la etiqueta corta `Número beneficiario` en `woocommerce_get_item_data`, para priorizar claridad comercial en checkout sin exponer textos técnicos.
- Regla de visibilidad en columna `Producto`: en ítems DingConnect, `woocommerce_cart_item_name` debe renderizar el nombre comercial y, debajo, una segunda línea corta con `Número beneficiario: <valor>` en formato HTML seguro (`esc_html`/`wp_kses`), para asegurar lectura inmediata en `Tu pedido`.
- Enforcement obligatorio en modo WooCommerce: `POST /wp-json/dingconnect/v1/transfer` debe responder `403` cuando `payment_mode=woocommerce`, de modo que la recarga real solo se ejecute desde el backend post-pago mediante `process_recarga_on_payment`.
...
- Hardening 29-04-2026 (checkout/cart): los ítems `dc_recarga` se rehidratan defensivamente desde sesión para restaurar `product_id` y objeto `data` válidos antes de validar carrito; además se registra diagnóstico dirigido en `debug.log` cuando WooCommerce detecta carrito inválido (`missing_product`, `not_purchasable`, `out_of_stock`) para acelerar parche final en producción.
- Hardening 29-04-2026 (checkout/cart, iteración 2): tras cargar carrito desde sesión, el plugin normaliza de nuevo ítems `dc_recarga` (`woocommerce_cart_loaded_from_session`) y emite traza heartbeat `[DingConnect][checkout_cart_validation]` en cada validación de checkout para confirmar que la versión desplegada realmente está ejecutando en servidor.
- Mitigación de falso positivo en checkout DC-only: si solo hay recargas DingConnect en carrito y WooCommerce publica el aviso genérico de "problemas con artículos del carrito" sin detalle operativo, ese notice genérico se limpia para evitar bloqueo por interferencias externas no relacionadas con stock/purchasable real del ítem.
- Endurecimiento de timing: la limpieza del notice genérico también se ejecuta al cierre de `woocommerce_check_cart_items` (prioridad 999), capturando avisos añadidos por plugins de terceros después de la primera pasada de validación.
- Endurecimiento de origen: el plugin filtra `woocommerce_add_error` para suprimir el mensaje genérico de "problemas con artículos del carrito" cuando el carrito es `DC-only` y la validación propia reporta `issues_count = 0`.
- Endurecimiento de matching: la supresión del aviso genérico ahora detecta variantes en español/inglés (incluyendo el texto de "volver al carrito") y aplica limpieza adicional al render/proceso de checkout para capturar inyecciones tardías.
- Endurecimiento de pasarelas: en modo WooCommerce para recargas, si `woo_allowed_gateways` no coincide con pasarelas disponibles en runtime, el plugin bloquea checkout antes del pago con notice operativo y deja traza `[DingConnect][checkout_gateways]`. No debe hacerse fallback a todas las pasarelas porque eso permite cobrar y luego bloquear el despacho por `blocked_gateway`.
- Observabilidad de bloqueos locales: si la guardia final de despacho detecta una pasarela no permitida, marca el ítem como `blocked_gateway`, cancela reintentos y crea entrada en `Transfer Logs` con contexto de pedido, ítem, método usado y pasarelas permitidas, aunque no exista llamada a DingConnect.
- Diagnóstico en producción: las trazas rutinarias de checkout solo deben escribirse con `WP_DEBUG=true`; las anomalías que requieren acción operativa sí quedan siempre en `debug.log`.
- Revisión preventiva: en `Credenciales > Pasarelas permitidas`, si aparecen IDs guardados sin coincidencia con WooCommerce, corregir la selección antes de ejecutar otra compra real.
- Hallazgo operativo confirmado en logs: coexistencia de `issues_count = 0` en validación DingConnect y warnings de Tropipay `session_start(): Session cannot be started after headers have already been sent`, indicando interferencia externa de sesión en checkout.
- Mitigación aplicada: inicialización temprana de sesión PHP en requests de checkout (`init` prioridad 0) y evaluación de contexto `DC-only` desde sesión WooCommerce para que la supresión de avisos genéricos siga operando aunque el carrito no llegue completamente hidratado.
- Mitigación adicional aplicada: exclusión de la clase de gateway Tropipay en `woocommerce_payment_gateways` para checkout `DC-only` cuando Tropipay no está permitido en configuración. Esta capa actúa antes de la lista de gateways disponibles y evita que Tropipay ejecute hooks/constructores durante ese checkout.
- Mitigación adicional aplicada (media thumbnails): los ítems `dc_recarga` ya no renderizan miniatura en carrito/checkout y el producto base fuerza `image_id=0`, evitando lecturas de metadatos de adjuntos corruptos que disparaban `Undefined array key "file"` y `preg_match(null)` en `wp-includes/media.php`.
- Hardening adicional del notice genérico: la detección del mensaje de WooCommerce ya contempla la variante productiva `Hay algunos problemas con los artículos de tu carrito... Por favor vuelve a la página del carrito... antes de pagar`, además de dejar traza diagnóstica cuando entra un notice no suprimido en checkout `DC-only`.
- Hallazgo adicional en producción: el log confirmó una regla externa de importe mínimo (`Para poder hacer un pedido, el total del carro de compra debe ser de al menos $50,00.`) que se estaba evaluando durante checkout. Para carritos exclusivamente de recarga DingConnect, ese notice se trata como interferencia externa y también se suprime para no bloquear el flujo payment-first.
- Configuración opcional de UX en checkout `DC-only`: el plugin puede ocultar el bloque de créditos de tienda de Advanced Coupons (`Apply store credit discounts`) para evitar fricción visual en recargas; esta ocultación se controla desde `Credenciales > Checkout DC-only: Store Credit` y no desactiva Advanced Coupons para el resto de la tienda.
