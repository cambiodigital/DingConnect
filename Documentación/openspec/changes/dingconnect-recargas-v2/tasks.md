...
- [x] 2.5 Harden wizard backend validation to prevent REST session step-skipping and enforce step prerequisites on save.
- [x] 2.6 Enable number-first automatic country detection (country optional in UI with backend inferred `country_iso` when available).
...
### 6.0 Matrix 6 Smoke Test — `scripts/run-matrix-6.ps1` (2026-04-17)
**Resultado: PASS=11 FAIL=0 SKIP=0** — verificación estructural/smoke sobre staging Docker.

| Check | Resultado | Qué verifica |
|-------|-----------|-------------|
| 6.1 `wizard/config → 200` | ✅ PASS | REST endpoint vivo y respondiendo |
| 6.2 `wizard/offers?category=recargas&entry_mode=country_fixed&country_iso=MX` | ✅ PASS | Endpoint alcanzable (500 esperado: API key no configurada en staging) |
| 6.2 `wizard/offers?category=gift_cards&entry_mode=country_fixed&country_iso=CU` | ✅ PASS | Endpoint alcanzable (idem) |
| 6.3 `PAYMENT_FIRST_OK` | ✅ PASS | Hooks `woocommerce_order_status_processing` y `_completed` registrados |
| 6.4 `IDEMPOTENCY_OK` | ✅ PASS | Método `is_item_already_successful()` existe en DC_Recargas_WooCommerce |
| 6.5 Gateway baseline: bacs, cheque, cod | ✅ PASS | Los 3 gateways habilitados en staging |
| 6.6 `wizard_enabled=1` | ✅ PASS | Opción persistida correctamente |
| 6.6 `validate_only=1` | ✅ PASS | Modo seguro activo |
| 6.6 `allow_real_recharge=0` | ✅ PASS | Transferencias reales bloqueadas en staging |
| 6.6 `retry_attempts=2` | ✅ PASS | Política de reintentos configurada |
| 6.7 Found 5 page(s) with [dingconnect] shortcodes | ✅ PASS | Páginas con shortcodes creadas en staging |

> **Nota**: La API key de DingConnect no está configurada en staging (`api_key: ''`).
> `wizard/offers` responde 500 con JSON estructurado `{"ok":false,"message":"No has configurado el API Key..."}`.
> El check 6.2 acepta cualquier HTTP ≠ 0 (endpoint existe y responde) — no es una prueba de integración real.

### E2E y Pruebas de Comportamiento (pendientes — requieren API key real o entorno de prueba DingConnect)
...
- [ ] 6.3 Validate payment-first guard (behavioral): pending/failed/canceled orders MUST NOT dispatch transfer in Woo hooks. _(hooks registrados ✅ — comportamiento sin verificar)_
- [ ] 6.4 Validate paid-state dispatch idempotency (behavioral): repeated status transitions MUST NOT duplicate transfers for same order item. _(método existe ✅ — lógica sin ejercitar)_
- [ ] 6.5 Run multi-gateway matrix (Stripe, PayPal, Redsys/Bizum or available equivalents) and record state transition evidence. _(bacs/cheque/cod verificados ✅ — gateways de pago reales pendientes)_
- [ ] 6.6 Verify retry policy + manual reconciliation path for transient provider errors and audit order notes/logs.
- [ ] 6.7 Validate shortcode variants and progressive enhancement fallback in at least one external landing theme. _(páginas con shortcodes existen ✅ — rendering en tema real pendiente)_
