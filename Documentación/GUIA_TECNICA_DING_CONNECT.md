...
- Regla de render del select de paquetes en frontend: para evitar que las opciones largas del desplegable nativo queden recortadas al borde derecho del contenedor, los wrappers principales del shortcode (`.dc-card` y `.dc-wizard`) deben permanecer con `overflow: visible`; en móvil (`max-width: 640px`) se usa fuente ligeramente menor y menor padding derecho del select para mejorar visibilidad de labels largos.
...
- Regla de estado activo en landings: cuando un bundle está desactivado en `Productos guardados`, permanece visible en los checklists de alta/edición de landing pero aparece marcado como `Desactivado` y no puede seleccionarse ni marcarse como destacado; además, el backend de alta/actualización solo acepta `bundle_ids` activos.
...
- Visibilidad de branding por producto en admin: la tabla de `Productos guardados` incluye columna `Logo URL` en primera posición de datos (después del checkbox de selección), mostrando enlace clicable cuando el bundle tiene `logo_url` y el fallback `Sin logo` cuando no existe.
...
- Regla visual para `Shortcodes creados`: la tabla debe mantener presentación minimalista y profesional (cabecera de alto contraste sobrio, celdas con espaciado uniforme, bordes suaves y badges discretos para metadatos) sin romper accesibilidad de foco/teclado ni interacciones por fila.
