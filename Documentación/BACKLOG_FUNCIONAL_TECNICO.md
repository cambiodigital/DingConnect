...
- **15 Mayo 2026 (v2.8.47):** Voucher (thank-you): se eliminó la fila "País" del comprobante (no se muestra en ningún caso).
- **15 Mayo 2026 (v2.8.46):** Checkout thank-you: se corrigió duplicación del aviso de éxito del comprobante, la fecha/hora del voucher se muestra en zona horaria España (Europe/Madrid) y se oculta la fila "País" cuando el valor viene vacío/`[]`.
...
20. Ajuste de copy en frontend público para confirmación de recargas móviles: el mensaje previo al checkout dejó de reutilizar lenguaje de estado pendiente del proveedor y ahora aclara al cliente que, tras confirmar, pasará al pago para completar la solicitud de recarga.
...
58bis. Ampliación de `Tipo de paquete` en `Catálogo y alta -> Buscar en API`: para `CU` (Cuba), si el campo `Operador` no contiene `cubacel`, el producto se clasifica y se muestra como `Tarjetas/Gifcards`.
...
100. Eliminación del operador en las opciones del select en frontend público: el nombre del proveedor se quitó de las opciones del select debajo de "PAQUETES DISPONIBLES" en el frontend, manteniendo únicamente el título del paquete y su precio para una visualización más limpia.
...
136. Checkout independiente para recargas con "Cart Swap": el flujo de recarga ahora aísla la compra haciendo un snapshot del carrito original (comida, etc.) y cupones en la sesión del cliente, permitiendo un checkout exclusivo para la recarga y restaurando silenciosamente el carrito original tras el pago exitoso, la cancelación explícita o el abandono de la página de checkout. v2.8.16.
...
147. Corrección de descripción en checkout para bundles guardados: la rehidratación de `bundle_benefit` en WooCommerce ahora consulta `dc_recargas_bundles` en lugar de `dc_recargas_options` y reemplaza beneficios degradados (`vacío`, igual al label o igual al SKU) por la descripción operativa persistida, para que la columna `Producto` muestre `Beneficios recibidos` cuando exista.
...
4. Versionado de release técnico por trazabilidad: cabecera del plugin y constante interna alineadas a `2.7.00` en `dingconnect-recargas.php`.
...
- Corrección post-incidente de pasarelas (02-05-2026): si `woo_allowed_gateways` queda configurado pero no coincide con ninguna pasarela disponible en checkout, el plugin ya no hace fallback a todas las pasarelas; bloquea el checkout antes del pago con notice accionable, registra traza diagnóstica y mantiene la guardia final `blocked_gateway` solo como última defensa.
- Observabilidad post-incidente (02-05-2026): los bloqueos `blocked_gateway` ahora generan entrada en `Transfer Logs` con `WP_Error` serializado (código, mensaje y contexto), aunque no se haya llamado a `SendTransfer`, para que la pestaña de registros no quede vacía ante fallos de política local.
- Corrección de rehidratación de carrito (02-05-2026): `normalize_dc_cart_after_session_load()` dejó de usar `$cart_item_key` fuera del `foreach`, eliminando warnings PHP y asegurando hidratación por ítem.
- Optimización de diagnóstico (02-05-2026): las trazas rutinarias de checkout DingConnect (`heartbeat`, notices suprimidos, sesión PHP temprana, gateways aplicadas) quedan condicionadas a `WP_DEBUG`; solo las anomalías reales, como pasarelas permitidas sin coincidencias disponibles, siguen escribiéndose siempre en `debug.log`.
- Prevención administrativa (02-05-2026): la pestaña `Credenciales > Pasarelas permitidas` muestra advertencia si hay IDs guardados que ya no aparecen entre las pasarelas registradas por WooCommerce.
